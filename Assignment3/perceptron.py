...
# SPECIFICATION: This Python script trains and evaluates a Single Layer Perceptron and a Multi-Layer Perceptron
#   on a dataset of optically recognized handwritten digits. The goal is to find the best combination of learning
#   rate and shuffle settings that gives the highest accuracy on a test set.
...
for learning in n: #iterates over n
    ...
    for shuffle in r: #iterates over r
        ...
        for algo in ['Perceptron', 'MLP']: #iterates over the algorithms
            ...
            #-->add your Pyhton code here
            if algo == 'Perceptron':
                clf = Perceptron(eta0=learning, shuffle=shuffle, max_iter=1000)
            else:
                clf = MLPClassifier(
                    activation='logistic',
            ...
            if algo == 'Perceptron':
                if accuracy > highest_perceptron_acc:
                    ...
                    print(f"Highest Perceptron accuracy so far: {accuracy:.4f}, Parameters: learning rate={learning}, shuffle={shuffle}")
            else:
                if accuracy > highest_mlp_acc:
                    highest_mlp_acc = accuracy
                    print(f"Highest MLP accuracy so far: {accuracy:.4f}, Parameters: learning rate={learning}, shuffle={shuffle}")
